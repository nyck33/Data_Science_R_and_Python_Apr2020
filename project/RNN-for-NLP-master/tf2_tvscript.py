...
def get_inputs():
    ...
    Input = tf.compat.v1.placeholder(tf.int32, [None, None], name='input')
...
def get_init_cell(batch_size, rnn_size):
    ...
    num_layers=2
    cell = tf.compat.v1.nn.rnn_cell.MultiRNNCell([tf.compat.v1.nn.rnn_cell.BasicLSTMCell(rnn_size) for _ in range(num_layers)])
...
def get_embed(input_data, vocab_size, embed_dim):
    ...
    """
    # TODO: Implement Function
    embedding = tf.Variable(tf.random.uniform((vocab_size, embed_dim), -1, 1))#embedding matrix
    embed = tf.nn.embedding_lookup(params=embedding, ids=input_data)#embedding tensors
...
def build_rnn(cell, inputs):
    ...
    """
    # TODO: Implement Function
    Outputs, final_state = tf.compat.v1.nn.dynamic_rnn(cell, inputs, dtype = tf.float32, initial_state = None)
...
with train_graph.as_default():
    ...
    input_data_shape = tf.shape(input=input_text)
